...
/**
 * @brief Performs basic tests for various linear algebra operations and neural network functions.
 
void basicTests()
 * @brief Tests the peekColumns function by extracting columns from a matrix and verifying their contents.
 
{
  //...

}
 * @brief Tests the pokeColumns function by modifying columns of a matrix with new values and verifying the changes.
 
{
  //...
}
 * @brief Tests various linear algebra operations such as matrix-vector multiplication, vector addition, subtraction, and scalar multiplication.
 
{
  //...
}
 * @brief Initializes matrices and vectors for testing purposes.
 
Matrix<FloatType> w1_init(3,2, vecD({0.1,0.2,
                             -0.1,-0.2,
                             0.7,0.7}));
Vector<FloatType> b1_init( vecD({0.5,0.7,0.9}));
 * @brief Verifies the initialization of w1_init and its column values.
 
doHost(w1_init, { 
  //...
});
 * @brief Extracts a column from w1_init using the peekColumn function and verifies its contents.
 
{
  auto c = peekColumn(w1_init,0);
  //...
}
 * @brief Computes the mean squared error cost function for a neural network layer.
 *
 * @param input_layer Input layer of the neural network.
 * @param w1_init Weights of the neural network layer.
 * @param b1_init Biases of the neural network layer.
 * @return Mean squared error cost function object.
 
auto f = mse_cost( dnn_layer(input_layer<FloatType>(), w1_init, b1_init) )
 * @brief Defines input and output data for testing the mean squared error cost function.
...
Matrix<FloatType> y1(3,2, vecD({-0.5, -0.5,
         1.7, 1.3,
         -0.7, -0.5}));
 * @brief Tests the computation of the mean squared error loss between predicted and actual outputs.
 
{
  //...
}
 * @brief Tests the computation of the derivatives of the mean squared error loss with respect to the model parameters.
 
{
  //...
}
 * @brief Updates the model parameters using the computed derivatives and tests the updated model.
