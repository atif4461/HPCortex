...
/**
 * @brief Tests dynamic models by comparing values obtained from manually composed layers with those from automatically wrapped layers.
 
void testDynamicModel() {
  //...
}
 * @brief Creates a simple neural network layer with one neuron and tests its value computation against expected results.
 *
 * This part of the test creates a small neural network consisting of multiple fully connected (dense) layers,
 * then compares the output of this model with that of a manually constructed equivalent model.
 
{
  //...
}
 * @brief Verifies the correctness of convolutional and dense layers composition through comparison of computed outputs.
 *
 * In this section, we construct a more complex model involving convolutional, flattening, and dense layers,
 * and validate its behavior by checking if the output matches expectations within a specified tolerance.
